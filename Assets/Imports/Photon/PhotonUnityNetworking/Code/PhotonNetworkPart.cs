...
namespace Photon.Pun {
    ...
    public static partial class PhotonNetwork {
        ...
        public static void AddCallbackTarget(object target) {
            if (target is PhotonView) return;
            ...
            if (punOwnershipCallback != null) {
                ...
                OnOwnershipTransferedEv += punOwnershipCallback.OnOwnershipTransferred;
        ...
        public static void RemoveCallbackTarget(object target) {
            if (target is PhotonView || NetworkingClient == null) return;
        ...
        public static float LevelLoadingProgress {
            get {
                ...
                else if (_levelLoadingProgress > 0f) _levelLoadingProgress = 1f;
        ...
        private static void LeftRoomCleanup() {
            ...
            if (autoCleanupSettingOfRoom || offlineModeRoom != null) LocalCleanupAnythingInstantiated(true);
        }
        ...
        internal static void LocalCleanupAnythingInstantiated(bool destroyInstantiatedGameObjects) {
            ...
            if (destroyInstantiatedGameObjects) {
                ...
                foreach (var go in instantiatedGos) RemoveInstantiatedGO(go, true);
            }
        ...
        private static void ResetPhotonViewsOnSerialize() {
            foreach (var photonView in photonViewList.Values) photonView.lastOnSerializeDataSent = null;
        }
...
#pragma warning restore 0414
        ...
        internal static void ExecuteRpc(Hashtable rpcData, Player sender) {
            ...
            if (rpcData.ContainsKey(keyByteOne)) otherSidePrefix = (short)rpcData[keyByteOne];
            ...
            if (rpcData.ContainsKey(keyByteFour)) arguments = (object[])rpcData[keyByteFour];

            var photonNetview = GetPhotonView(netViewID);
            if (photonNetview == null) {
                var viewOwnerId = netViewID / MAX_VIEW_IDS;
            ...
            if (LogLevel >= PunLogLevel.Full) Debug.Log("Received RPC: " + inMethodName);


            // SetReceiving filtering
            if (photonNetview.Group != 0 &&
                !allowedReceivingGroups.Contains(photonNetview.Group)) return; // Ignore group
            ...
            var receivers = 0;
            var foundMethods = 0;
            if (!UseRpcMonoBehaviourCache || photonNetview.RpcMonoBehaviours == null ||
                photonNetview.RpcMonoBehaviours.Length == 0) photonNetview.RefreshRpcMonoBehaviourCache();
            ...
            for (var componentsIndex = 0; componentsIndex < photonNetview.RpcMonoBehaviours.Length; componentsIndex++) {
                ...
                if (cachedRPCMethods == null) continue;
                    ...
                    if (arguments == null) {
                        if (parameters.Length == 0) {
                            ...
                            if (RunRpcCoroutines) {
                                ...
                                if ((ie = o as IEnumerator) != null) PhotonHandler.Instance.StartCoroutine(ie);
                            }
                        }
                            ...
                            receivers++;
                            var o = mInfo.Invoke((object)monob,
            ...
            if (receivers != 1) {
                ...
                if (argumentsTypes != null) {
                    ...
                    for (var index = 0; index < argumentsTypes.Length; index++) {
                        ...
                        if (argsString != string.Empty) argsString += ", ";
        ...
        private static bool CheckTypeMatch(ParameterInfo[] methodParameters, Type[] callParameterTypes) {
            if (methodParameters.Length < callParameterTypes.Length) return false;
            ...
            for (var index = 0; index < callParameterTypes.Length; index++) {
                ...
                if (callParameterTypes[index] != null && !type.IsAssignableFrom(callParameterTypes[index]) &&
                    !(type.IsEnum && Enum.GetUnderlyingType(type).IsAssignableFrom(callParameterTypes[index])))
                    return false;
#endif
            }
        ...
        public static void DestroyPlayerObjects(int playerId, bool localOnly) {
            ...
            foreach (var view in photonViewList.Values) {
                ...
                if (view.OwnerActorNr == playerId) {
                    ...
                    if (OnOwnershipTransferedEv != null) OnOwnershipTransferedEv(view, previousOwner);
                }
            }

            // any non-local work is already done, so with the list of that player's objects, we can clean up (locally only)
            foreach (var gameObject in playersGameObjects) RemoveInstantiatedGO(gameObject, true);
        }
        ...
        internal static void RemoveInstantiatedGO(GameObject go, bool localOnly) {
            ...
            if (!localOnly) ServerCleanInstantiateAndDestroy(viewZero); // server cleaning
            ...
            for (var j = foundPVs.Count - 1; j >= 0; j--) {
                ...
                if (view.InstantiationId >= 1) LocalCleanPhotonView(view);
                if (!localOnly) OpCleanRpcBuffer(view);
            }

            if (LogLevel >= PunLogLevel.Full) Debug.Log("Network destroy Instantiated GO: " + go.name);
        ...
        private static readonly Hashtable removeFilter = new();
        private static readonly Hashtable ServerCleanDestroyEvent = new();

        private static readonly RaiseEventOptions ServerCleanOptions =
            new() { CachingOption = EventCaching.RemoveFromRoomCache };
        ...
        internal static void RequestOwnership(int viewID, int fromOwner) {
            ...
            RaiseEventInternal(PunEvent.OwnershipRequest, new int[] { viewID, fromOwner }, SendToAllOptions,
        ...
        internal static void TransferOwnership(int viewID, int playerID) {
            ...
            RaiseEventInternal(PunEvent.OwnershipTransfer, new int[] { viewID, playerID }, SendToAllOptions,
                SendOptions.SendReliable);
        }
        ...
        public static void RegisterPhotonView(PhotonView netView) {
            ...
            if (LogLevel >= PunLogLevel.Full) Debug.Log("Registered PhotonView: " + netView.ViewID);
        }
        ...
        public static bool RemoveBufferedRPCs(int viewId = 0, string methodName = null,
            ...
            if (viewId != 0) filter[keyByteZero] = viewId;
            ...
            if (callersActorNumbers != null) raiseEventOptions.TargetActors = callersActorNumbers;
        ...
        private static Hashtable rpcEvent = new();

        private static RaiseEventOptions RpcOptionsToAll = new();


        internal static void RPC(PhotonView view, string methodName, RpcTarget target, Player player, bool encrypt,
            params object[] parameters) {
            if (blockedSendingGroups.Contains(view.Group)) return; // Block sending on this group
            ...
            if (view.Prefix > 0) rpcEvent[keyByteOne] = (short)view.Prefix;
            ...
            if (parameters != null && parameters.Length > 0) rpcEvent[keyByteFour] = (object[])parameters;
            ...
            switch (target) {
                ...
                case RpcTarget.AllViaServer: {
                    ...
                    RaiseEventInternal(PunEvent.RPC, rpcEvent, options, sendOptions);
                    if (OfflineMode) ExecuteRpc(rpcEvent, NetworkingClient.LocalPlayer);
        ...
        public static void SetInterestGroups(byte[] disableGroups, byte[] enableGroups) {
            ...
            if (disableGroups != null) {
                if (disableGroups.Length == 0)
                    ...
                    for (var index = 0; index < disableGroups.Length; index++) {
                        ...
                        if (allowedReceivingGroups.Contains(g)) allowedReceivingGroups.Remove(g);
                    }
            }
            ...
            if (enableGroups != null) {
                if (enableGroups.Length == 0) {
                    ...
                    for (byte index = 0; index < byte.MaxValue; index++) allowedReceivingGroups.Add(index);
            ...
            if (!offlineMode) NetworkingClient.OpChangeGroups(disableGroups, enableGroups);
        }
        ...
        internal static void NewSceneLoaded() {
            ...
            foreach (var kvp in photonViewList) {
                var view = kvp.Value;
                if (view == null) removeKeys.Add(kvp.Key);
            }
        ...
        private class SerializeViewBatch : IEquatable<SerializeViewBatch>, IEquatable<RaiseEventBatch> {
            ...
            public SerializeViewBatch(RaiseEventBatch batch, int offset) {
                ...
                this.offset = offset;
                for (var i = 0; i < offset; i++) ObjectUpdates.Add(null);
            }
            ...
            public bool Equals(RaiseEventBatch other) {
                return Batch.Reliable == other.Reliable && Batch.Group == other.Group;
            }

            public override bool Equals(object obj) {
                var other = obj as SerializeViewBatch;
                return other != null && Batch.Equals(other.Batch);
            }
        ...
        internal static void RunViewUpdate() {
            if (OfflineMode || CurrentRoom == null || CurrentRoom.Players == null) return;


            // no need to send OnSerialize messages while being alone (these are not buffered anyway)
#if !PHOTON_DEVELOP
            if (CurrentRoom.Players.Count <= 1) return;
#else
 ...
 (CurrentRoom.Players.Count == 1) ? ReceiverGroup.All : ReceiverGroup.Others;
            ...
            while (enumerator.MoveNext()) {
                ...
                if (view.Synchronization == ViewSynchronization.Off || view.IsMine == false ||
                    view.isActiveAndEnabled == false) continue;

                if (blockedSendingGroups.Contains(view.Group)) continue; // Block sending on this group


                // call the PhotonView's serialize method(s)
                var evData = OnSerializeWrite(view);
                if (evData == null) continue;
                ...
                svBatch.Add(evData);
                if (svBatch.ObjectUpdates.Count == svBatch.ObjectUpdates.Capacity) SendSerializeViewBatch(svBatch);
            }

            var enumeratorB = serializeViewBatches.GetEnumerator();
            while (enumeratorB.MoveNext()) SendSerializeViewBatch(enumeratorB.Current.Value);
        }


        private static void SendSerializeViewBatch(SerializeViewBatch batch) {
            if (batch == null || batch.ObjectUpdates.Count <= 2) return;
            ...
            RaiseEventInternal(code, batch.ObjectUpdates, serializeRaiseEvOptions,
        ...
        private static List<object> OnSerializeWrite(PhotonView view) {
            if (view.Synchronization == ViewSynchronization.Off) return null;


            // each view creates a list of values that should be sent
            var info = new PhotonMessageInfo(NetworkingClient.LocalPlayer, ServerTimestamp, view);
            ...
            if (serializeStreamOut.Count <= SyncFirstValue) return null;
            ...
            if (view.Synchronization == ViewSynchronization.Unreliable) return currentValues;
            ...
            if (view.Synchronization == ViewSynchronization.UnreliableOnChange) {
                if (AlmostEquals(currentValues, view.lastOnSerializeDataSent)) {
                    if (view.mixedModeIsReliable) return null;
        ...
        private static void OnSerializeRead(object[] data, Player sender, int networkTime, short correctPrefix) {
            ...
            // SetReceiving filtering
            if (view.Group != 0 && !allowedReceivingGroups.Contains(view.Group)) return; // Ignore group
        ...
        private static object[] DeltaCompressionRead(object[] lastOnSerializeDataReceived, object[] incomingData) {
            ...
            if (lastOnSerializeDataReceived == null) return null;
        ...
        private static bool AlmostEquals(IList<object> lastData, IList<object> currentContent) {
            if (lastData == null && currentContent == null) return true;

            if (lastData == null || currentContent == null || lastData.Count != currentContent.Count) return false;

            for (var index = 0; index < currentContent.Count; index++) {
                var newObj = currentContent[index];
                var oldObj = lastData[index];
                if (!AlmostEquals(newObj, oldObj)) return false;
            }
        ...
        private static bool AlmostEquals(object one, object two) {
            ...
            if (!one.Equals(two)) {
                ...
                if (one is Vector3) {
                    var a = (Vector3)one;
                    var b = (Vector3)two;
                    if (a.AlmostEquals(b, PrecisionForVectorSynchronization)) return true;
                }
                    ...
                    if (a.AlmostEquals(b, PrecisionForQuaternionSynchronization)) return true;
                }
                else if (one is float) {
                    var a = (float)one;
                    var b = (float)two;
                    if (a.AlmostEquals(b, PrecisionForFloatSynchronization)) return true;
                }
        ...
        internal static bool GetMethod(MonoBehaviour monob, string methodType, out MethodInfo mi) {
            ...
            if (monob == null || string.IsNullOrEmpty(methodType)) return false;
        ...
        internal static void LoadLevelIfSynced() {
            if (!AutomaticallySyncScene || IsMasterClient || CurrentRoom == null) return;

            // check if "current level" is set in props
            if (!CurrentRoom.CustomProperties.ContainsKey(CurrentSceneProperty)) return;

            // if loaded level is not the one defined by master in props, load that level
            var sceneId = CurrentRoom.CustomProperties[CurrentSceneProperty];
            if (sceneId is int) {
                if (SceneManagerHelper.ActiveSceneBuildIndex != (int)sceneId) LoadLevel((int)sceneId);
            }
            else if (sceneId is string) {
                if (SceneManagerHelper.ActiveSceneName != (string)sceneId) LoadLevel((string)sceneId);
            }
        }
        ...
        internal static void SetLevelInPropsIfSynced(object levelId) {
            ...
            if (CurrentRoom.CustomProperties.ContainsKey(CurrentSceneProperty)) {
                var levelIdInProps = CurrentRoom.CustomProperties[CurrentSceneProperty];
            ...
            switch (photonEvent.Code) {
                ...
                case PunEvent.CloseConnection:
                    ...
                    if (EnableCloseConnection == false)
                        ...
                        Debug.LogWarning("CloseConnection received from " + originatingPlayer +
                                         ". That player is not the Master Client. " + MasterClient + " is.");
                    else if (EnableCloseConnection) LeaveRoom(false);

                    break;

                case PunEvent.DestroyPlayer:
                    var evData = (Hashtable)photonEvent.CustomData;
                    ...
                    switch (requestedView.OwnershipTransfer) {
                        case OwnershipOption.Takeover:
                            ...
                            if (requestedFromOwnerId == currentPvOwnerId ||
                                ...
                                if (OnOwnershipTransferedEv != null) OnOwnershipTransferedEv(requestedView, prevOwner);
                            }
                            else {
                        ...
                        case OwnershipOption.Request:
                            if (OnOwnershipRequestEv != null) OnOwnershipRequestEv(requestedView, originatingPlayer);
                            break;
            ...
            switch (opResponse.OperationCode) {
                case OperationCode.GetRegions:
                    ...
                    if (ConnectMethod == ConnectMethod.ConnectToBest) {
                        var previousBestRegionSummary = BestRegionSummaryInPreferences;
                ...
                case OperationCode.JoinGame:
                    if (Server == ServerConnection.GameServer) LoadLevelIfSynced();
                    break;
            }
        }
        ...
        private static void OnRegionsPinged(RegionHandler regionHandler) {
            if (LogLevel >= PunLogLevel.Informational) Debug.Log(regionHandler.GetResults());
            ...
            if (!string.IsNullOrEmpty(PhotonServerSettings.DevRegion) && ConnectMethod == ConnectMethod.ConnectToBest) {
                ...
                NetworkingClient.ConnectToRegionMaster(_finalDevRegion);
                return;
            }
#endif
