...
public class VRKeyboardManager : MonoBehaviour {
    ...
    private void Update() {
        ...
        keyboardPosition = Vector3.Lerp(keyboardPosition,
            playerCamera.transform.position + playerCamera.transform.rotation * _angleRotator * relativePosition,
            keyboardLerpSpeed);
        keyboardPosition.y = keyboardPosition.y < 0 ? 0 : keyboardPosition.y;

        _keyboard.transform.position = keyboardPosition;
        _keyboard.transform.rotation = Quaternion.Lerp(_keyboard.transform.rotation,
            playerCamera.transform.rotation * _keyboardRotator, keyboardLerpSpeed);
    }
    ...
    public void EnableVRKeyboard(int inputFieldNumber) {
        if (IsKeyboardActive()) {
            ...
            _keyboard.transform.position = playerCamera.transform.position +
                                           playerCamera.transform.rotation * _angleRotator * relativePosition;
